...
app.delete('/api/events/:id', (req, res) => {
 ...
 console.log('Delete attempt - Password received:', password); // Additional logging

 if (password !== 'ceelovesya') {
   console.log('Delete failed - Incorrect password:', password); // Additional logging
   return res.status(401).json({ error: 'Incorrect password' });
 }
 ...
 db.run('DELETE FROM events WHERE id = ?', [eventId], (err) => {
   ...
   console.log('Event deleted successfully:', eventId); // Additional logging
